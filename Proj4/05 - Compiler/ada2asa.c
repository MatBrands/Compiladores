...
void start_asa(){
    FILE *file;
    file = fopen("ada_tree", "r");
    fscanf(file, "%d\n", &tam_syntax);
    Syntax syntax[tam_syntax];
    for (int i=0; i<tam_syntax; i++){
        if (i<tam_syntax-1){
            fscanf(file, "%d | %c\n", &syntax[i].indice, &syntax[i].token);
        }
        else{
            fscanf(file, "%d | %c", &syntax[i].indice, &syntax[i].token);
        }
    }
    fclose(file); file = NULL;

    Asa_Tree asa_tree[tam_syntax];
...
void ada2asa(Syntax *syntax, Asa_Tree *asa, int atual){
...
void place_token(Syntax *syntax, Asa_Tree *asa, int atual){
    int token_atual = 0;

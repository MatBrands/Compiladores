...
int main(){
    ...
    code[1].f = LIT; code[1].l = 0; code[1].a = 4;         // Valor que será calculado no fat
    code[2].f = STO; code[2].l = 0; code[2].a = 4 + 3;     // Passamos como parâmetro
    code[3].f = CAL; code[3].l = 0; code[3].a = 6;         // Chamamos a função fat
    code[4].f = LOD; code[4].l = 0; code[4].a = 4 + 3;     // Pegamos o retorno da função
    ...
    code[10].f = JPC; code[10].l = 0; code[10].a = 13;     // Se for dará jump no else para prosseguir para n*fat(n-1);
    // if
    code[11].f = STO; code[11].l = 0; code[11].a = 3;      // Armazena o valor 1 para retorno
    code[12].f = OPR; code[12].l = 0; code[12].a = 0;      // Retorno da subrotina
    // else
    code[13].f = LOD; code[13].l = 0; code[13].a = 3;      // Carrega o n
    code[14].f = LIT; code[14].l = 0; code[14].a = 1;      // Carrega 1 para fazer a subtração (n-1)
    code[15].f = OPR; code[15].l = 0; code[15].a = 3;      // n-1
    code[16].f = STO; code[16].l = 0; code[16].a = 4 + 3;  // Passa como parâmetro
    code[17].f = CAL; code[17].l = 0; code[17].a = 6;      // Chama a função para fat(n-1)
    code[18].f = LOD; code[18].l = 0; code[18].a = 4 + 3;  // Pegamos o retorno da função
    code[19].f = LOD; code[19].l = 0; code[19].a = 3;      // Carrega o n atual
    code[20].f = OPR; code[20].l = 0; code[20].a = 4;      // Faz a multiplicação n*fat(n-1)
    code[21].f = STO; code[21].l = 0; code[21].a = 3;      // Salva o retorno
    code[22].f = OPR; code[22].l = 0; code[22].a = 0;      // Retorno da subrotina
